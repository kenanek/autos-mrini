...
@section('breadcrumb', 'Gestión / Añadir Marca')

@section('content')
<div class="page-header" style="margin-bottom: 24px;">
    <h1 class="page-title">Añadir Marca</h1>
    <p class="page-subtitle">Registra una nueva marca de vehículos en el sistema.</p>
</div>
...
<form action="{{ route('admin.brands.store') }}" method="POST">
    ...
    <div class="card" style="overflow: hidden; background: white;">
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div style="grid-column: 1 / -1;">
                        ...
                        <div class="input-wrapper"><input type="text" name="name" class="form-control" value="{{ old('name') }}" placeholder="Ej: Toyota, BMW, Mercedes-Benz" required></div>
                    </div>
                    <div>
                        <label class="form-label">País de Origen</label>
                        <div class="input-wrapper"><input type="text" name="country" class="form-control" value="{{ old('country') }}" placeholder="Ej: Alemania, Japón, España"></div>
                    </div>
                    <div>
                        <label class="form-label">Estado</label>
                        <select name="is_active" class="form-control">
                            <option value="1" @if(old('is_active', 1) == 1) selected @endif>Activo</option>
                            <option value="0" @if(old('is_active', 1) == 0) selected @endif>Inactivo</option>
                        </select>
                    </div>
                </div>
            </div>
        ...
        <div style="padding: 24px; border-top: 1px solid #e2e8f0; background: #f8fafc; display: flex; justify-content: flex-end; gap: 16px;">
            ...
            <button type="submit" class="btn btn-primary" style="padding:12px 32px;"><i class="icon-save" style="margin-right:8px;"></i> Guardar Marca</button>
        </div>
    </div>
</form>
@endsection